...

{
    type WeekEvents = {
        ...
        MorningCamps: RawCampData[],
        AfternoonCamps: RawCampData[],
        AllDayCamps: RawCampData[]
    }

    let campSpecificEvents: WeekEvents[] = [];
    ...
    for (let campSpecificEvent of campSpecificEvents){
        ...
        if (campSpecificEvent.StartDate.Month == campSpecificEvent.EndDate.Month){
            head.innerText = `${calendarLookup[<6 | 7>campSpecificEvent.StartDate.Month]} ${campSpecificEvent.StartDate.Day} - ${campSpecificEvent.EndDate.Day}`;
        }else{
            head.innerText = `${calendarLookup[<6 | 7>campSpecificEvent.StartDate.Month]} ${campSpecificEvent.StartDate.Day} - ${calendarLookup[<6 | 7>campSpecificEvent.EndDate.Month]} ${campSpecificEvent.EndDate.Day}`;
        }
