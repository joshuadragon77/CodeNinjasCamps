...
type CampState = "AllDay" | "Morning" | "Afternoon";
type Days = "Sunday" | "Monday" | "Tuesday" | "Wednesday" | "Thursday" | "Friday" | "Saturday";
type FrequencyType = "Sunday" | "Monday" | "Tuesday" | "Wednesday" | "Thursday" | "Friday" | "Saturday";
type ExtraStyles = "Cut" | "Normal";
class Camp{
    name: string;
    campLink: string;
    startDate: Date;
    ...
    endingTime: Date = new Date();
    campState: CampState;
    extraStyles: ExtraStyles;

    constructor(name: string, campLink: string, startDate: Date, endDate: Date, campState: CampState, extraStyles: ExtraStyles = "Normal"){
        this.name = name;
...
function setUpCalendar(calendarElement: HTMLDivElement, camps: Camp[], startingDay: Days, endDate: number){

    let tableBody = calendarElement.querySelector("tbody")!;

    let physicsBody: (HTMLElement | null)[][] = [];
    let colorsWeek: string[][] = []

    for (let i = 0;i<endDate;i++){
        physicsBody.push([]);
    }

    let startingDateIndex = <number>{
        "Sunday": 0,
        "Monday": 1,
        "Tuesday": 2,
        "Wednesday": 3,
    ...
    let dataAssignmentTable: HTMLTableCellElement[] = [];
    ...
    function weekNumberFromDay(date: number){
    ...
    for (let camp of camps){
        ...
        function attemptToPileChild(child: HTMLAnchorElement, selectedDateIndex: number){
...
function translateRawData(dataElement: RawCampData, filterMonthIndex: number){
    ...
    let campState: CampState = "Morning";
    ...
    let extraStyles: ExtraStyles = "Normal";
...
}
{
    ...
    setUpCalendar(
        <HTMLDivElement>document.querySelector("#June"),
        juneSpecificEvents, "Thursday", 30
    )
}
{
    ...
    setUpCalendar(
        <HTMLDivElement>document.querySelector("#July"),
        julySpecificEvents, "Saturday", 31
    )
}
{
    ...
    setUpCalendar(
        <HTMLDivElement>document.querySelector("#August"),
        augustSpecificEvents, "Tuesday", 31
    )
}